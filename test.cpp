...
public:
	...
	class Path_message
	{
		...
		friend Path_message operator + (const Path_message &u, const Path_message &v)
		{
	...
	public:
		...
		friend inline Lcc_message operator + (const Lcc_message &u, const Lcc_message &v)
		{
            Lcc_message res;
            Edge *e = u.last_edge;
            res.path_msg = u.path_msg * Path_message(e -> w) * v.path_msg;
	...
	public:
	    ...
	    void maintain()
	    {
            ...
            if(ls) message = ls -> message + message;
		...
		void zig()
		{
			...
			if (fa -> par)
			{
				if (fa -> par -> ls == fa) fa -> par -> ls = this;
				else if (fa -> par -> rs == fa) fa -> par -> rs = this;
			}
			par = fa -> par;
			fa -> par = this;
			fa -> ls = rs;
	...
	public:
		int wa, wb;
		Weight() {}
		Weight(const int &wa, const int &wb) : wa(wa), wb(wb) {}
...
int main()
{
	...
	while(Q--)
	{
		...
		{
			scanf("%d%d%d%d", &u, &v, &wa, &wb);
			if (lcc.cut(u, v, wa, wb)) printf("ok\n");
			else printf("failed\n");
		}
		else if (op[0] == 'a')
		{
			scanf("%d%d%d", &u, &v, &delta);
			if (lcc.add(u, v, delta)) printf("ok\n");
			else printf("failed\n");
		}
		else if (op[0] == 'd')
		{
			...
			std::pair<int, int>ret = lcc.query(u, v);
